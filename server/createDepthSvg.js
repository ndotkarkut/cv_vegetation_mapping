...
const makeSvgBench = (bench, color) => {
  ...
  return `<image x="${xCoord}" y="${yCoord}" width="${20}" height="${20}" style="color:${
    color || "#FF0000"
  } fill:${
    color || "#FF0000"
  }"  href="${"https://cdn-icons-png.flaticon.com/512/1024/1024639.png"}" />`;
};
...
const getBenchPositions = (bench, yMax, maxBenchDepth) => {
  ...
  const norm_depth =
    (fakeDepthScaling(bench.depth, maxBenchDepth) / (yMax + 2)) * 240;
...
const drawRoadLine = (heading) => {
  ...
  return `<line x1="${250 + x}" y1="${250 + y}" x2="${250 - x}" y2="${
    250 - y
  }" style="stroke: #FF0000 "></line>`;
};

const getGreeneryOnRoad = (heading, reverseHeading) => {
  console.log("heading", heading);
  heading = heading - 90;
  reverseHeading = reverseHeading - 90;
  const range = [degToRad(heading - 15), degToRad(heading + 15)];
  const flippedRange = [
    degToRad(reverseHeading - 15),
    degToRad(reverseHeading + 15),
  ];

  console.log(range, flippedRange);

  return [range, flippedRange];
};
...
module.exports = async (panoId, heading) => {
  ...
  const objectsDetected = JSON.parse(objectDetection);
  let bench = objectsDetected.bench || {};
  let coordinates = bench.coordinates || [];
  ...
  const reverseHeading =
    180 + +heading > 360 ? (180 + heading) % 360 : 180 + heading;
  ...
  const [roadRadius1, roadRadius2] = getGreeneryOnRoad(heading, reverseHeading);

  for (let idx in shiftedXValues) {
    const theta = shiftedXValues[idx];
    // if (
    //   (theta > roadRadius1[0] && theta < roadRadius1[1]) ||
    //   (theta > roadRadius2[0] && theta < roadRadius2[1])
    // )
    //   continue;
    svg += makeSvgPoint(
      theta,
